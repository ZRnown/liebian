...
```bash
# 创建项目目录（根据你的实际路径调整）
sudo mkdir -p /www/wwwroot/liebian.mifzla.top
sudo chown -R $USER:$USER /www/wwwroot/liebian.mifzla.top
cd /www/wwwroot/liebian.mifzla.top
```
...
```bash
# 如果有Git仓库
git clone <your-repo-url> /www/wwwroot/liebian.mifzla.top
cd /www/wwwroot/liebian.mifzla.top
```
...
```bash
# 从本地使用SCP上传
scp -r /本地项目路径/* user@服务器IP:/www/wwwroot/liebian.mifzla.top/
```
...
PAYMENT_CONFIG = {
    ...
    'notify_url': 'http://liebian.mifzla.top:5051/api/payment/notify',  # 回调地址
    'return_url': 'http://liebian.mifzla.top:5051/payment/success',  # 返回地址
    'pay_type': 'trc20',  # 支付类型
    'version': '1.0'
}
```
...
```bash
sudo nano /etc/nginx/sites-available/liebian.mifzla.top
```
...
server {
    listen 80;
    server_name liebian.mifzla.top;
...
```bash
sudo ln -s /etc/nginx/sites-available/liebian.mifzla.top /etc/nginx/sites-enabled/
sudo nginx -t  # 测试配置
...
# 获取证书
sudo certbot --nginx -d liebian.mifzla.top
...
Type=simple
User=你的用户名
WorkingDirectory=/www/wwwroot/liebian.mifzla.top
ExecStart=/usr/bin/python3 /www/wwwroot/liebian.mifzla.top/a.py
Restart=always
RestartSec=10
StandardOutput=append:/www/wwwroot/liebian.mifzla.top/bot.log
StandardError=append:/www/wwwroot/liebian.mifzla.top/bot.log
...
数据库使用 SQLite3，文件位置：`/www/wwwroot/liebian.mifzla.top/bot.db`
...
# 备份数据库（每天执行）
cp /www/wwwroot/liebian.mifzla.top/bot.db /backup/telegram-bot/bot_$(date +%Y%m%d).db
...
# 添加以下行（每天凌晨2点备份）
0 2 * * * cp /www/wwwroot/liebian.mifzla.top/bot.db /backup/telegram-bot/bot_$(date +\%Y\%m\%d).db
```
...
```bash
# 查看实时日志
tail -f /www/wwwroot/liebian.mifzla.top/bot.log

# 查看最近100行
tail -n 100 /www/wwwroot/liebian.mifzla.top/bot.log

# 清空日志（谨慎操作）
> /www/wwwroot/liebian.mifzla.top/bot.log
