...
from telethon.tl.types import (
    ...
    UpdateChatParticipantAdmin, UpdateChannelParticipant
)

from config import (
    API_ID, API_HASH, USE_PROXY,
    PROXY_TYPE, PROXY_HOST, PROXY_PORT, DATA_DIR, SESSION_DIR
)
from database import get_db_conn, get_cn_time
...
def get_main_account_id(telegram_id, username=None):
    ...
    try:
        ...
        if not row:
            c.execute(
                'SELECT main_account_id FROM fallback_accounts WHERE telegram_id = ? AND main_account_id IS NOT NULL LIMIT 1',
                (telegram_id,)
            )
...
async def notify_group_binding_invalid(chat_id, bot_id=None, reason="群组状态异常", notify_bot=None):
    ...
    try:
        ...
        if not bound_users:
            print(f'[通知] ❌ 未在数据库中找到绑定群组 {chat_id} 的用户')
            return
    ...
    try:
        ...
        if update_type == 'UpdateChatParticipantAdmin':
            ...
            if was_admin and not is_now_admin:
                permission_changed = True
                print(f"[Raw权限检测] 🚨 频道/超群管理员权限被撤销: 用户 {target_user_id} @ 群 {target_chat_id}")
...
async def check_and_notify_permission_change(bot, user_id, chat_id, update_type):
    ...
    try:
        ...
        if not is_admin:
            ...
            conn = get_db_conn()
            c = conn.cursor()
            c.execute('UPDATE member_groups SET is_bot_admin = 0 WHERE telegram_id = ? AND (group_id = ? OR group_id = ?)',
                     (user_id, chat_id, full_chat_id))
            c.execute('UPDATE members SET is_bot_admin = 0 WHERE telegram_id = ?', (user_id,))
            conn.commit()
            conn.close()
    except Exception as e:
        print(f'[权限检查] 验证失败: {e}')
        await notify_group_binding_invalid(chat_id, user_id, "机器人权限验证失败", bot)
            ...
            try:
                ...
                try:
                    ...
                    if not is_admin:
                        ...
                        global permission_check_triggered
                        permission_check_triggered = True
                        await notify_group_binding_invalid(group_id, user_id, "定期检查发现管理员权限被撤销", target_bot)
                        conn = get_db_conn()
                        c = conn.cursor()
                        c.execute('UPDATE member_groups SET is_bot_admin = 0 WHERE telegram_id = ? AND group_id = ?', (user_id, group_id))
                        c.execute('UPDATE members SET is_bot_admin = 0 WHERE telegram_id = ?', (user_id,))
                        conn.commit()
                        conn.close()
            ...
            for telegram_id, group_link, db_is_bot_admin in members:
                try:
                    ...
                    if original_is_bot_admin == 1 and current_is_bot_admin == 0:
                        ...
                        await notify_group_binding_invalid(group_identifier, telegram_id, "检测到机器人管理员权限被撤销", notify_bot)
    ...
    try:
        ...
        async def sync_after_start():
            ...
            try:
                ...
                await sync_member_groups_from_members(clients)
            except Exception as e:
                print(f"同步失败: {e}")
...
# 导入支付模块
from payment import create_recharge_order, PAYMENT_CONFIG
    ...
    try:
        ...
        if isinstance(event_or_id, int):
            ...
            try:
                ...
                event_or_id.sender_id = get_main_account_id(original_sender_id, getattr(original.sender, 'username', None))

            except Exception:

                pass

            member = DB.get_member(event_or_id.sender_id)

            telegram_id = event_or_id.sender_id
...
# 所有VIP开通路径都统一调用 distribute_vip_rewards，删除所有手写分红代码
...
async def profile_handler(event):
    ...
    text += f'💎 VIP状态: {"✅" if member["is_vip"] else "❌"}\n'
...
async def join_upline_cmd(event):
    ...
    await handle_join_upline(event, event.client, DB)
...
async def admin_handler(event):
    ...
    buttons = [
        ...
        [Button.inline('👩‍💼 设置客服文本', b'admin_set_support'), Button.inline('📢 用户广播', b'admin_broadcast')],

        [Button.inline('🎁 手动充值VIP', b'admin_manual_vip'), Button.inline('📊 查看会员统计', b'admin_stats')]
        ...
        if link.startswith('http://t.me/') or link.startswith('https://t.me/'):
            ...
            if verification_result['success']:
                ...
                try:
                    ...
                    upsert_member_group(sender_id, link, sender_username, is_admin_flag, group_id=group_id)
...
async def check_member_status_task():
    ...
    while True:
        ...
        try:
            ...
            # 如果有权限变化触发器被设置，立即检查，否则等待30秒

            if permission_check_triggered:

                print("[状态检测] 🔄 检测到权限变化，立即执行检查...")

                permission_check_triggered = False  # 重置触发器
                    ...
                    if current_is_joined_upline == 1:
                        ...
                        if upline_groups_to_check:
                            ...
                            try:
                                ...
                                upline_chat = await bot.get_participants(upline_group_username, limit=1000)

                                member_ids = [p.id for p in upline_chat]
