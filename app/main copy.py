...
from core_functions import (
    ...
    update_level_path, get_fallback_account, distribute_vip_rewards, get_cn_time
)
    ...
    try:
        ...
        if member:
            ...
            if is_vip_order and new_balance >= config['vip_price'] and not member['is_vip']:
                ...
                # 【核心修改】调用统一的分红函数
                stats = await distribute_vip_rewards(bot, telegram_id, amount, config)
...
💵 您的余额: {user_balance} U"""
    ...
    if referrer_id:
        # 使用核心函数获取上级链 (包括捡漏补位的账号)
        chain = get_upline_chain(telegram_id, 10)
        
        for item in chain:
            # 只显示非捡漏账号的真实上级的群（如果上级符合条件）
    ...
    # --- B. 获取捡漏/推荐群 (Fallback Groups) ---
    fb_groups_raw = get_fallback_resource('group')
    if fb_groups_raw:
        text += "🔥 **推荐加入的群组：**\n"
        # 处理可能的分隔符
        fb_list = [g.strip() for g in fb_groups_raw.split('\n') if g.strip()]
    ...
    buttons.append([Button.inline('🔍 验证已加群', f'verify_groups_{telegram_id}'.encode())])
...
async def confirm_vip_callback(event):
    ...
    DB.update_member(event.sender_id, balance=new_balance, is_vip=1, vip_time=get_cn_time())
    
    # 更新层级路径
    update_level_path(event.sender_id)
    
    # 【核心修改】调用统一的分红函数
    stats = await distribute_vip_rewards(bot, event.sender_id, config['vip_price'], config)
...
async def message_handler(event):
    ...
    if sender_id in admin_waiting:
        ...
        if wait_type == 'level_count':
            ...
            # 开通VIP (使用 get_cn_time 统一时间)
            DB.update_member(target_user['telegram_id'], is_vip=1, vip_time=get_cn_time())
            
            # 更新层级路径
            update_level_path(target_user['telegram_id'])
            
            # 【关键修改】调用统一分红函数 (替代旧的几百行代码)
            stats = await distribute_vip_rewards(bot, target_user['telegram_id'], config['vip_price'], config)
...
def update_payment_config():
    ...
    try:
        ...
        # 更新各个配置项（不包含 payment_usdt_address，只使用实时解析）
